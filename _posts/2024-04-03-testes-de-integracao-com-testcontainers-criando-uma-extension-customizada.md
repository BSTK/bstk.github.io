...
Estou usando o mesmo projeto do post anterior, então caso queira acompanhar e seguir a linha de raciocio é interessante dar uma lida antes de iniciar esse aqui.
...
Agora remova o caso de teste (o método com anotado com ```@Test```), deixando a classe apenas com o código de configuração. Neste ponto, deve estar dessa forma:
...
- Na sessão ```ASSERTS```, faço minhas asserções.
  ...
  - Primeiro: Confirmos que a ```response``` que foi retornada tenha os mesmo dados da qual eu enviei
...
Neste ponto, estamos com praticamente duas classes de testes idênticas e todo código de configuração é iguais, ou seja **duplicado** e isso não é bom. Vamos refatorar e criar uma classe que servirá apenas para as configurações, assim podemos ```extende-la```, deixando nossas classes de teste apenas com o código que é de seu interesse, os casos de teste.
...
Pronto, podemos executar cada uma delas que tudo continuará funcionando numa boa, agora sem a repetição de código e repetir código nunca é bom, mesmo que seja o código de teste.
...
- O método ```close()``` essem sim será executado quando todas as classes de testes terminar de executar, novamente um ponto ideal para finalizarmos o container:
```java
AppTestContainer.stopContainer();
```

Para que isso seja feito assim foi preciso ajustar alguns pontos na classe ```AppTestContainer```, e os pontos são esses:
